...
def loadBoxSizes(config_data, sequences):
    ...
    for idx in tqdm(range(len(sequences))):
        ...
        for box_i in gt_instances["cart_boxes"]:
            ### this is for [x_min, x_max, y_min, y_max, z_min, z_max] ###
            all_boxes.append(box_i[2:])
...
def getOptimalAnchorsWithPlot(gt_boxes, centers, stopping_err_portion=0.85):
    ...
    drawer.barChart(centers_explored, centers_errs, axes[0], \
                    title="K means errors (For Cartesian Boxes)", \
                    xlabel="Number of kmeans centers", \
                    ylabel="Values of kmeans errors", color="blue", \
                    xtricks=centers_explored)
    drawer.saveFigure("../images", "anchor_kmeans_cartboxes.png")
    return previous_center

def saveAnchorsAsTxt(anchors):
    """ Save anchor boxes to txt file """
    with open("../anchors_cartboxes.txt", "w") as f:
